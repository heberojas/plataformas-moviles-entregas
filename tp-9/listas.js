...
function triplicarLista(listaDeNumeros) {
    return listaDeNumeros.map((elemento)=>elemento * 3);
}
...
function filtrarSoloPositivos(listaDeNumeros) {
    return listaDeNumeros.filter((elemento) => elemento > 0);
}
...
function filtrarSoloTruthy(listaDeValores) {
    return listaDeValores.filter((elemento) => Boolean(elemento));
}
...
function enumerarLista(listaDePalabras) {
    ...
    let primerasPalabras = listaDePalabras.slice(0, -1).join(", ");
    let ultimaPalabra = listaDePalabras[listaDePalabras.length - 1];
    return primerasPalabras + " y " + ultimaPalabra + ".";
}
